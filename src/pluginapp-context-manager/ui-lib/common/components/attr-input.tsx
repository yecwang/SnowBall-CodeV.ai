...
export default function AttrInput({ unit, label, initialValue, onChange, attributeKey, type, icon, sx }: TProps) {
  const _handleValue = (v: string | number | undefined) => {
  ...
  return (
    <Tooltip title={label} arrow>
      <TextField
        ...
        multiline
        onBlur={() => onChange({ [attributeKey]: unit ? `${_handleValue(value)}${unit}`: _handleValue(value) })}
        onKeyDown={(event) => {
          if (event.key === 'Enter') {
            event.preventDefault();
            onChange({ [attributeKey]: unit ? `${_handleValue(value)}${unit}`: _handleValue(value) });
          }
        }}
      />
    </Tooltip>
